...
Zygos is a lightweight, containerized API service that leverages a local GPU and an AI model to provide intelligent context verdicts on message content. Built with FastAPI for high performance, it runs its AI inference via an Ollama model (gemma3 - 4 Billion parameters) hosted in a Podman container, and uses ngrok to securely expose the local endpoint to the web.
